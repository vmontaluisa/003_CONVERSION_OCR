...
METADATA_DIR = os.path.join(BASE_DIR, "04_JSON_METADATA")
OCR_DIR = os.path.join(BASE_DIR, "05_OCR")
LOG_FILE = os.path.join(BASE_DIR, "00_LOGS", "repository_log")
INDEX_FILE = os.path.join(BASE_DIR, "index.json")
OPENAI_API_KEY="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"
OPENAI_MODELO="gpt-3.5-turbo"
LENGUAJE="spa"
MAXIMO_TEXTO_OPENAI=500
...
def extract_metadata_and_text(pdf_path):
    ...
    try:
        ...
        metadata_adicional = extract_metadata_txt_with_openai(text, PREGUNTAS)
...
#            "extracted_text": text[:5000]  # Guardar un fragmento del texto extraído
        ...
        respuesta.update(metadata_adicional)
...
def extract_metadata_txt_with_openai(text, questions):
        try:
            ...
            log_action(f"Error al procesar metadata con OpenAI en {text}: {e}")
            return None
        
  

def extract_metadata_file_with_openai(text_path, questions):
    """Envía el texto a OpenAI para extraer metadatos específicos de forma estricta y devuelve JSON."""
    try:
        client = OpenAI(api_key=OPENAI_API_KEY)
        ...
        log_action(f"Error al procesar metadata con OpenAI en {text_path}: {e}")
        return None
        

def process_new_pdfs():
    """Procesar PDFs nuevos, aplicar OCR si es necesario y extraer metadatos."""
    for filename in tqdm( os.listdir(NEW_DIR) , desc="Procesando PDFs", unit="archivo")  :
        if filename.endswith(".pdf"):
