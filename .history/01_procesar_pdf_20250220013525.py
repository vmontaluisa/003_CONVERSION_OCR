...
def extract_metadata_with_openai(text_path, questions):
    """Envía el texto a OpenAI para extraer metadatos específicos según un arreglo de preguntas."""
    try:
        client = OpenAI(api_key=OPENAI_API_KEY)
        with open(text_path, "r", encoding="utf-8") as f:
            text = f.read()
        
        prompt = """Extrae la siguiente información del texto:
        """ + "\n".join([f"- {q}" for q in questions]) + "\n\n" + text
        
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[{"role": "system", "content": "Eres un asistente que extrae información clave de documentos y devuelveme en formato json."},
